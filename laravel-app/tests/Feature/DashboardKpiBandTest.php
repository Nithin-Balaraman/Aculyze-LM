...
class DashboardKpiBandTest extends TestCase
{
    ...
    public function test_proposal_outcome_chart_respects_employee_scoping(): void
    {
        ...
        $nithinLead = Lead::create(['prospect_id' => $nithinProspect->id, 'assigned_to' => $nithin->id, 'created_by' => $nithin->id, 'stage' => 'validated', 'temperature' => 'hot']);
        $kuralLead = Lead::create(['prospect_id' => $kuralProspect->id, 'assigned_to' => $kural->id, 'created_by' => $kural->id, 'stage' => 'validated', 'temperature' => 'hot']);
