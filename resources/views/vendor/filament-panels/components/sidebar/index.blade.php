{{--
    Overrides vendor/filament/filament/resources/views/components/sidebar/index.blade.php
    (filament/filament v3.3.54) — byte-for-byte identical except for ONE
    addition inside the header, marked below: a small icon-only "A" mark
    (public/images/aculyze_mark.png — the same asset already derived for
    the favicon, not a new image) shown ONLY in the collapsed-desktop
    state.

    Filament's own header already hides the ENTIRE logo wrapper div below
    (`x-show="$store.sidebar.isOpen"`) when the sidebar is collapsed —
    confirmed empirically (Logo sizing refinement follow-up): collapsing
    leaves just the bare expand-chevron button, no branding at all. There
    is no separate "collapsed logo" slot anywhere in Filament core to hook
    into, so the only way to add one is this full-file view override
    (the same customization technique already used elsewhere in this app —
    see resources/views/vendor/filament-tables/components/selection/).

    On any Filament upgrade, diff this file against the new vendor
    version and re-apply just the marked block.
--}}
@props([
    'navigation',
])
...
>
    <div class="overflow-x-clip">
        ...
        >
            ...
            {{-- ================= START: Aculyze-added block ================= --}}
            @if (filament()->isSidebarCollapsibleOnDesktop())
                <img
                    src="{{ asset('images/aculyze_mark.png') }}"
                    alt="{{ __('filament-panels::layout.logo.alt', ['name' => filament()->getBrandName()]) }}"
                    x-cloak
                    x-show="! $store.sidebar.isOpen"
                    class="fi-logo-collapsed mx-auto"
                />
            @endif
            {{-- ================= END: Aculyze-added block ================= --}}
