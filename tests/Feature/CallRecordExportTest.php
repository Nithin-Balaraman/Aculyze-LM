 ...
 * (visibleTo($user)->directlyLogged()), so the privacy guarantee and the
 * "no follow-up-generated calls" exclusion apply identically to both paths.
 */
class CallRecordExportTest extends TestCase
{
    ...
    /**
     * Call Records generated by a Follow-Up's "Completed" action (see
     * FollowUp::generatedCallRecord()) exist purely to drive
     * CallRoutingService and must have zero visibility anywhere — including
     * exports. CallRecordExporter::scopedQuery() applies ->directlyLogged()
     * for exactly this reason, mirroring CallRecordResource's own
     * getEloquentQuery().
     */
    public function test_export_excludes_call_records_generated_by_a_completed_follow_up(): void
    {
        $employee = User::factory()->create();
        ...
        $this->assertStringNotContainsString('Generated by follow-up completion', $content);
    }
